...
export function validateEvidence(evidence, sourceCommit, manifest) {
    ...
    for (const run of evidence.runs) {
        ...
        if (run.status !== 'passed' || typeof run.executorVersion !== 'string' || !run.executorVersion.trim() ||
            ...
            !manifest.games[run.gameId].placeIds.includes(run.placeId) || !keysEqual(run.checks, REQUIRED_CHECKS)) fail('Incomplete runtime run: ' + identity);
    ...
    if (GAME_IDS.some(gameId => !coveredGames.has(gameId))) fail('Missing runtime game coverage');
}
